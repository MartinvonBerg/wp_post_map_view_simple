...
//			JB.LoadScript(JB.GPX2GM.Path+'gra_svg.js', function(){ JB.Scripte.gra = 2; });
	...
	function showRoutes() {
		...
		for(var i=0;i<gpxdaten.routen.anzahl;i++) if(chkrt.status[gpxdaten.routen.anzahl==1?0:i+1]) {
			...
			var delta = this.parameters.routenmarker;
...
//		}
	...
	function track_click_fkt(map,daten,point) {
		...
		//map.infowindow(info, a); // Martin
	} // track_click_fkt
	
	function trackinfo(tracki) { // Anzeige Trackdaten in Infobox links oben
		var info = "<strong>Trackdaten:</strong>"; // Martin
